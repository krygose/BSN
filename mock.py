...
class MockSerial:
    ...
    def write(self, data):
        print(f"[MOCK] Sent: {data.hex()}")
    
    def readline(self):
        return b"[MOCK] Response from device\n"
    
    def __enter__(self):
        print(f"[MOCK] Opened connection on {self.port} at {self.baudrate} baudrate")
        return self
    
    def __exit__(self, exc_type, exc_value, traceback):
        print("[MOCK] Closed connection")
